...
def iExit():
    iExit = tkinter.messagebox.askyesno(
        "Student Database", "Confirm if you want to exit")
    if iExit > 0:
        root.destroy()
        return
...
def updateData():
    if(len(txtstdAdmission.get()) != 0):
        Database.deleteRec(sd[0])
    if(len(txtstdAdmission.get()) != 0):
        Database.addStudentRecord(txtstdAdmission.get(), txtfullname.get(), txtDoB.get(
        ), txtAge.get(), comboGender.get(), txtParents_Contact.get(), comboclass.get())
        studentlist.delete(0, END)
        studentlist.insert(END, (txtstdAdmission.get(), txtfullname.get(), txtDoB.get(
        ), txtAge.get(), comboGender.get(), txtParents_Contact.get(), comboclass.get()))
...
lblAge = Label(DataFrameLeft, font=("aerial", 15, "bold"),
...
lblGender = Label(DataFrameLeft, font=("aerial", 15, "bold"),
...
txtParents_Contact = Label(DataFrameLeft, font=(
    "aerial", 15, "bold"), text="Parents Contact:", padx=2, pady=2, bg="Grey")
txtParents_Contact.grid(row=5, column=0, sticky=W)
txtParents_Contact = Entry(DataFrameLeft, font=(
    "Comfortaa Light", 15, "bold"),  width=39)
txtParents_Contact.grid(row=5, column=1, sticky=W)

current_class = Label(DataFrameLeft, font=(
    "aerial", 15, "bold"), text="Current Grade:", padx=2, pady=2, bg="Grey")
current_class.grid(row=6, column=0, sticky=W)
comboclass = Combobox(DataFrameLeft, font=(
    "Comfortaa Light", 15, "bold"), width=39)
comboclass["values"] = ("Grade 1", "Grade 2", "Grade 3", "Grade 4",
                        "Grade 5", "Grade 6", "Grade 7", "Grade 8", "PP1", "PP2")
...
# BUTTON WIDGTES
btnAddData = Button(ButtonFrame, text="Add New", font=(
    "aerial", 20, "bold"), width=10, height=1, bd=4, command=addData)
btnAddData.grid(row=0, column=0)

btnDisplayData = Button(ButtonFrame, text="Display", font=(
    "aerial", 20, "bold"), width=10, height=1, bd=4, command=DispalyData)
btnDisplayData.grid(row=0, column=1)

btnClearData = Button(ButtonFrame, text="Clear", font=(
...
btnSearchData = Button(ButtonFrame, text="Search", font=(
...
btnExit = Button(ButtonFrame, text="Exit", font=(
    "aerial", 20, "bold"), width=10, height=1, bd=4, command=iExit)
